...
#import "ZWDownloadQueueManager.h"
...
- (void)nextWaitTaskReZWme
{
    if (self.nextWaitDownloadinginfo) {
        ...
        info.oldBytes = 0;
        if (info.resumeData) {
            info.downloadTask = [[ZWURLSessionManager sharedInstance]downloadTaskWithResumeData:info.resumeData progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            info.downloadTask = [[ZWURLSessionManager sharedInstance]downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
                info.downloadBytes = downloadProgress.completedUnitCount;
                info.totalBytes = downloadProgress.totalUnitCount;
                NSDate *currentDate = [NSDate date];
                if ([currentDate timeIntervalSinceDate:info.startDate] >= 1) {
